...
static struct i2c_client_address_data addr_data = { .forces = forces, };
	...
	static void cypress_touch_esd_func(struct work_struct *work);
#endif
...
static int cyttsp_read_reg(struct i2c_client *client,u8 addr, u8 *pdata)
{
	...
	if (ret < 0)
	{
		printk("[CY8CTST242][PROXIMITY]i2c read error!ret:%d\n", ret);
		return ret;
	}
...
static int cyttsp_write_reg(struct i2c_client *client,u8 addr, u8 para)
{
	...
	if (ret < 0) {
		printk("[CY8CTST242][PROXIMITY]i2c write error!ret:%d,addr:%x,para:%x\n", ret,buf[0],buf[1]);
		return -1;
	}
...
void cyttsp_print_reg(struct i2c_client *client)
{
	...
	printk("[CY8CTST242][I2C]register read status1:%x,status2:%x\n",status1,status2);
	for(i=0; i<16;i++)
		printk("[CY8CTST242][I2C]addr:%02x,para:%02x", i,buffer[i]);
	printk("\n");
}
...
void enter_boot_mode(void)
{
	...
	mdelay(10);
	cyttsp_sw_reset();
	printk("[CY8CTST242][FIRMWARE]enter into bootloader mode!\n");
}
...
void exit_boot_mode(void)
{
	...
	printk("[CY8CTST242][FIRMWARE]exit from bootloader mode!\n");
	#else
	char buffer[2];
	int status=0;
	...
	if (status < 0)
	{
		printk("[CY8CTST242][REGISTER]exit_boot_mode failed 1st\n");
	}
	else
	{
		if (buffer[0]&0x10)
		{
			...
			if( status < 0)
			{
				printk("[CY8CTST242][REGISTER]exit_boot_mode failed 2nd\n");
			}
			else
			{
				printk("[CY8CTST242][REGISTER]exit_boot_mode succeeded\n");
			}
			msleep(300);
			status = i2c_smbus_read_i2c_block_data(i2c_client, 0x01, 1, &(buffer[0]));
			if (status < 0) {
...
void esd_check(void)
{
	if(need_rst_flag)
	{
		...
		{
			printk("[CY8CTST242][ESD]esd_check\n");
			//mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);  
			tp_reseting_flag = 1;
			cyttsp_sw_reset();
			tp_reseting_flag = 0;
...
void esd_thread(void)
{
	...
	while(1)
	{
		printk("[CY8CTST242][ESD]esd_thread, need_rst_flag=%d, fp_count=%d\n", need_rst_flag,fp_count);  
		fp_count = 0;
		#if 0
		if(need_rst_flag)
		{
...
{  
	...
	printk("[CY8CTST242][ESD]%s, line %d----\n", __FUNCTION__, __LINE__);

	esd_task = kthread_create(esd_thread, NULL, "esd_task");  

	if(IS_ERR(esd_task)){  
		printk("[CY8CTST242][ESD]failed to init esd thread\n");  
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	if (g_bPsSensorOpen)
	{
		...
		printk("[CY8CTST242][PROXIMITY] nPsDate=%d g_bPsSensorOpen=%d,g_nPsSensorDate=%d\n",g_bPsSensorOpen,g_nPsSensorDate,nPsDate);
		if(nPsDate)
		{		
		...
		{
			//printk("[CY8CTST242][PROXIMITY]g_bPsSensorOpen=%d,g_nPsSensorDate=%d\n",g_bPsSensorOpen,g_nPsSensorDate );
		...
		{
			extern void ctp_ps_event_handler(void);
			printk("[CY8CTST242][PROXIMITY] ctp_ps_event_handler");
			ctp_ps_event_handler();
...
{
	//mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
	//printk("[CY8CTST242][FUNCTION]tpd_flag:%d\n",tpd_flag);
	tp_interrupt_flag = 1;
	tpd_flag = 1;
...
static void ctp_power_on(kal_bool on)
{
	...
	{		
		...
		{
			printk("[ctp_power_on] Fail to enable MT65XX_POWER_LDO_VGP1 power\n");
		}

		//if(TRUE != hwPowerOn(MT6323_POWER_LDO_VCAM_AF, VOL_1800,"TP"))
		//{
		//	printk("[ctp_power_on] Fail to enable MT6323_POWER_LDO_VCAM_AF power\n");
		//}            
	...
	else
	{
		if(TRUE != hwPowerDown(MT6323_POWER_LDO_VGP2, "TP")) {
			printk("[ctp_power_on] Fail to disable MT65XX_POWER_LDO_VGP1 power\n");
		}
		//if(TRUE != hwPowerDown(MT6323_POWER_LDO_VCAM_AF,"TP")) {
		//	printk("[ctp_power_on] Fail to disable MT6323_POWER_LDO_VCAM_AF power\n");
		//}
	}
}
...
{
	printk("[CY8CTST242][PROXIMITY] tst242_get_ps %d",g_nPsSensorDate);
...
static int cyps242_enable_ps(int enable)
{
	...
	printk("[CY8CTST242][PROXIMITY]enable:%d, current state:%d\n", enable, g_bPsSensorOpen);
	if(enable == 1)
	{
		if(g_bPsSensorOpen == 0)
		{	
			u8 nSetPsEnable;
			printk("[CY8CTST242][PROXIMITY]do enable:%d, current state:%d\n", enable, g_bPsSensorOpen);
			g_bPsSensorOpen = 1;
...
//	wake_lock_init(&ps_lock,WAKE_LOCK_SUSPEND,"ps wakelock"); //shaohui add
	...
	{
		printk("[CY8CTST242][PROXIMITY]failed to init proximity function\n");
		return;
	}*/
}
#endif /* TPD_PROXIMITY */
...
{
	int cnt= 0;
	printk("[CY8CTST242][ENGINEER]Enter ctp_proc_read.\n");
	if(off != 0)
		return 0;
...
//	printk("vid:0x%04x,firmware:0x%04x\n",curr_ic_major, curr_ic_major);
	printk("[CY8CTST242][ENGINEER]Leave ctp_proc_read. cnt = %d\n", cnt);
	return cnt;
}
#endif
...
{	 
	...
	printk("[CY8CTST242][FUNCTION]enter %s: %d",__FUNCTION__, __LINE__);
	...
	if(status < 0) {
		printk("[CY8CTST242][FUNCTION]fail to prob cy8ctst242 1st\n");
		status = i2c_smbus_read_i2c_block_data(i2c_client, 0x00, 8, &(buffer[0]));
		if(status < 0) {
			printk("[CY8CTST242][FUNCTION]fail to prob cy8ctst242 2nd\n");
			return status;
		}
	}

	//cyttsp_has_bootloader  = ((buffer[1] & 0x10) >> 4);
	//printk("[CY8CTST242][FUNCTION]cyttsp_has_bootloader=%d\n",cyttsp_has_bootloader);
	...
	do {
		...
		printk("[CY8CTST242][ENGINEER]module:%x,firmware version:%x:\n",cyttsp_vendor_id,cyttsp_firmware_version);
	} while(0);
#endif
...
//	msleep(1000);
    ...
    if (IS_ERR(thread)) { 
		...
		printk("[CY8CTST242][FUNCTION]failed to create kernel thread: %d\n", retval);
		return retval;
    }
    ...
    tpd_load_status = 1;
		...
		if (g_ctp_proc == NULL) {
			printk("[CY8CTST242][ENGINEER]create_proc_entry failed\n");
		} else {
			g_ctp_proc->read_proc = ctp_proc_read;
			g_ctp_proc->write_proc = NULL;
			//g_ctp_proc->owner = THIS_MODULE;
			printk("[CY8CTST242][ENGINEER]create_proc_entry success\n");
		}
#endif
    printk("[CY8CTST242][FUNCTION]cyttsp tpd_i2c_probe success!!\n");
   ...
   //zx++
	...
	sprintf(tpd_desc, "%s-%s(0x%x)","Cypress", tpd_device_driver.tpd_device_name,
        cy8ctst_get_fw());
...
#ifdef ESD_CHECK	
	INIT_DELAYED_WORK(&esd_work, cypress_touch_esd_func);
...
{	
	...
	int ret = 0;
	//ret = cy8ctst_read_reg(CY8CTST_FW_REG, &fw);
	ret = cyttsp_read_reg(i2c_client, 0x1c, &status);
	printk("[CY8CTST242]cypress_touch_esd_func status_cnt:%d status:%d\n", status_cnt,status);
	if(ret < 0) {
		printk("[CY8CTST242]cypress_touch_esd_func i2c read error!ret:%d\n", ret);
		ret = cyttsp_read_reg(i2c_client, 0x1c, &status);
		if(ret < 0)
		{
			printk("[CY8CTST242]cypress_touch_esd_func reset TP 1!\n");
			ctp_power_on(0);
			msleep(200);
			ctp_power_on(1);
			queue_delayed_work(esd_wq, &esd_work, delay);
	...
	else
	{
		...
		if(status_cnt==status)
		{
			printk("[CY8CTST242]cypress_touch_esd_func reset TP 2!\n");
			ctp_power_on(0);
			msleep(200);
			ctp_power_on(1);
		}
...
{
	printk("Cypress cy8ctst242 I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
...
//	int status = 0;
	...
	{
		//printk("msg sensor resume in calling tp no need to resume\n");
		return;
	}
	//g_bSuspend = 0;
#endif
...
static void tpd_suspend(struct i2c_client *client, pm_message_t message)
{
	...
	if(g_bPsSensorOpen == 1)
	{
		//printk("msg suspend in calling tp no need to suspend\n");
		//g_bSuspend = 0;
		return;
	}
	//g_bSuspend = 1;
			...
			int show_status, int show_version, int show_cid)
{
	...
	if (show) {
	...
	#endif
		if (show_status) {
			printk("[CY8CTST242][FIRMWARE]BL%d: f=%02X s=%02X err=%02X bl=%02X%02X bld=%02X%02X\n", \
				show, \
				g_bl_data.bl_file, \
				g_bl_data.bl_status, \
				g_bl_data.bl_error, \
		...
		if (show_version) {
			printk("[CY8CTST242][FIRMWARE]BL%d: ttspver=0x%02X%02X appid=0x%02X%02X appver=0x%02X%02X\n", \
				show, \
		...
		if (show_cid) {
			printk("[CY8CTST242][FIRMWARE]BL%d: cid=0x%02X%02X%02X\n", \
				show, \
				g_bl_data.cid_0, \
				g_bl_data.cid_1, \
				g_bl_data.cid_2);
...
static int flash_block(u8 *blk, int len)
{
	...
	for (i = 0; i < len; i++, p += 2)
		sprintf(p, "%02x", blk[i]);
	printk("[CY8CTST242][FIRMWARE]%s: size %d, pos %ld payload %s\n",__func__, len, (long)0, buf);
	...
	if (retval < 0) {
		printk("[CY8CTST242][FIRMWARE]%s: failed\n", __func__);
		return retval;
	}
...
static int flash_data_rec(u8 *buf)
{
	...
	if (rc < 0) {
		printk("[CY8CTST242][FIRMWARE]%s: record ignored %s", __func__, buf);
		return 0;
	}
...
static int cyttspfw_flash_firmware(const u8 *data, int data_len)
{
	...
	if (!buf) {
		printk("[CY8CTST242][FIRMWARE]%s: no memory\n", __func__);
		return -ENOMEM;
	}
...
static void cyttspfw_flash_start(const u8 *data,int data_len, u8 *buf, bool force)
{
	...
	if (rc < 0) {
		printk("[CY8CTST242][FIRMWARE]failed to get hex firmware version\n");
		return;
	}
	...
	if (rc < 0) {
		printk("[CY8CTST242][FIRMWARE]failed to reset CHIP 1st\n");
		msleep(1000);
		rc = cyttsp_soft_reset();
	}
	...
	if(rc < 0)
	{
		...
		printk("[CY8CTST242][FIRMWARE]firmware update start tries:%d\n",tries);

		//rc  = i2c_smbus_read_i2c_block_data(i2c_client,CY_REG_BASE,8, tmpData);  
		//if(rc  < 0)
		//{
		//	printk("[CY8CTST242][FIRMWARE]failed to read version and module\n");
		//}
		//else
		//{
		//    printk("[CY8CTST242][FIRMWARE]succeeded to read version and module tmpData[0]:%x,tmpData[1]:%x\n",tmpData[0],tmpData[1]);
		//}
	...
	if (rc < 0) {
		printk("[CY8CTST242][FIRMWARE]cyttsp_soft_reset  try again later\n");
		exit_boot_mode();
		return;
	}

	printk("[CY8CTST242][FIRMWARE]Current firmware:%x.%x\n", g_bl_data.appver_hi, g_bl_data.appver_lo);
	printk("[CY8CTST242][FIRMWARE]New firmware:%x.%x\n", appver_hi, appver_lo);
	...
	if (fw_upgrade) {
		printk("[CY8CTST242][FIRMWARE]firmware upgrade process start\n");
		

		rc = cyttspfw_flash_firmware(data, data_len);
		if (rc < 0)
			printk("[CY8CTST242][FIRMWARE]failed to process firmware upgrade\n");
		else
			printk("[CY8CTST242][FIRMWARE]succeeded to process firmware upgrade\n");
		...
		if((fw_upgrade == 1) && (rc >= 0))
		{
			...
			else
			{
			   ...
			   printk("[CY8CTST242][FIRMWARE]succeeded to read version and module module is:%x,firmware version:%x:\n",tmpData[11],tmpData[12]);
			}
		}
	}
...
// endif
	...
	if(tpd_driver_add(&tpd_device_driver) < 0)
        printk("add CY8CTST242 driver failed\n");
    return 0;
}

/* should never be called */
static void __exit tpd_driver_exit(void) {
    printk("MediaTek CY8CTST242 touch panel driver exit\n");
#ifdef I2C_DMA_MODE
// add by gpg
		if(CTPI2CDMABuf_va)
		{
