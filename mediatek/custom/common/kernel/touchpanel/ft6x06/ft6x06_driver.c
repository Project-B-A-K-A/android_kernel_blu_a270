...
//	tpd_dump_touchinfo( ts );
 ...
 {	 
	...
	if (tpd_proximity_flag == 1)
	{
		...
		TPD_PROXIMITY_DBG("0xB0 state value is 0x%02X\n", state);
		...
		TPD_PROXIMITY_DBG("0x01 value is 0x%02X\n", proximity_status);
		...
		TPD_PROXIMITY_DBG("tpd_proximity_detect = %d\n", tpd_proximity_detect);

		if ((err = tpd_read_ps()))
		{
			TPD_PROXIMITY_DBG("read ps data 1156: %d\n", err);	
		}
		...
		{
			TPD_PROXIMITY_DBG("call hwmsen_get_interrupt_data failed= %d\n", err);	
		}
	}  
#endif
...
//zx++
	...
	sprintf(tpd_desc, "%s-%s(0x%x)","FT", tpd_device_driver.tpd_device_name,
        panel_version);
...
//BEIN <tp> <DATE20130514> <tp proximity> zhangxiaofei
	...
	TPD_PROXIMITY_DBG("attach start = %d\n", err);
	if((err = hwmsen_attach(ID_PROXIMITY, &obj_ps)))
	{
		TPD_PROXIMITY_DBG("attach fail = %d\n", err);
	}
	else
	{
		TPD_PROXIMITY_DBG("attach ok = %d\n", err);
	}	
#endif	
//END <tp> <DATE20130514> <tp proximity> zhangxiaofei
	return 0;
...
//BEIN <tp> <DATE20130514> <tp proximity> zhangxiaofei
   ...
   if (tpd_proximity_flag == 1){
	   if(tpd_proximity_flag_one == 1){
		   ...
		   TPD_PROXIMITY_DBG(TPD_DEVICE "tpd_proximity_flag_one \n");
		   return;
		   }
	}
#endif   
...
static int tpd_enable_ps(int enable)
{
	...
	i2c_smbus_read_i2c_block_data(g_pts->client, TPD_PROXIMITY_ENABLE_REG, 1, &state);
	TPD_PROXIMITY_DBG("read: 0xb0's value is 0x%02X\n", state);
	if (enable){
		state |= 0x01;
		tpd_proximity_flag = 1;
		TPD_PROXIMITY_DBG("ps function is on\n");	
	}else{
		state &= 0x00;	
		tpd_proximity_flag = 0;
		TPD_PROXIMITY_DBG("ps function is off\n");
	}

	ret = i2c_smbus_write_i2c_block_data(g_pts->client, TPD_PROXIMITY_ENABLE_REG, 1, &state);
	TPD_PROXIMITY_DBG("write: 0xB0's value is 0x%02X\n", state);
	return 0;
}
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	hwm_sensor_data *sensor_data;
	TPD_PROXIMITY_DBG("command = 0x%02X\n", command);		
	switch (command)
	{
		case SENSOR_DELAY:
			if((buff_in == NULL) || (size_in < sizeof(int)))
			{
				TPD_PROXIMITY_DBG("Set delay parameter error!\n");
				err = -EINVAL;
			}
			// Do nothing
			break;
		...
		case SENSOR_ENABLE:
			if((buff_in == NULL) || (size_in < sizeof(int)))
			{
				TPD_PROXIMITY_DBG("Enable sensor parameter error!\n");
				err = -EINVAL;
			}
			else
			{				
				...
				{		
					...
					{
						TPD_PROXIMITY_DBG("enable ps fail: %d\n", err); 
						return -1;
					}
				}
				else
				{
					...
					{
						TPD_PROXIMITY_DBG("disable ps fail: %d\n", err); 
						return -1;
					}
				}
			}
		...
		case SENSOR_GET_DATA:
			...
			{
				TPD_PROXIMITY_DBG("get sensor data parameter error!\n");
				err = -EINVAL;
			}
			else
			{
				...
				{
					sensor_data->values[0] = tpd_get_ps_value();
				    TPD_PROXIMITY_DBG("sensor_data->values[0] = %d\n", sensor_data->values[0]);
					sensor_data->value_divide = 1;
					sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
				}	
			...
			break;
		default:
			TPD_PROXIMITY_DBG("proxmy sensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
