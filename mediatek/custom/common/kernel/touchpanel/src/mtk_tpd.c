...
int tpd_driver_add(struct tpd_driver_t *tpd_drv)
{
	...
	for(i = 1; i < TP_DRV_MAX_COUNT; i++)
	{
		...
		if(tpd_driver_list[i].tpd_device_name == NULL)
		{
			...
			tpd_driver_list[i].tpd_get_ps = tpd_drv->tpd_get_ps;
			tpd_driver_list[i].tpd_convert = tpd_drv->tpd_convert;
			#if 0
...
static ssize_t tpd_desc_show(struct device *dev,
					...
					char *buf)
{
	TPD_DMESG("tpd_desc_show %s\n", tpd_desc); 
	return sprintf(buf, "%s\n", tpd_desc);
}
static DEVICE_ATTR(tpd_fw_vendor_info, 0444, tpd_desc_show, NULL);
//zx-- 20130814
...
//#endif
	...
	if(device_create_file(&pdev->dev, &dev_attr_tpd_fw_version)) {
		TPD_DMESG("LIMI==> create fw_version file error\n");
	}
	if(device_create_file(&pdev->dev, &dev_attr_tpd_fw_vendor_info)) {
		TPD_DMESG("LIMI==> create touch_info file error\n");
	}
    //zx-- 20130814
